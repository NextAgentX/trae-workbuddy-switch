...
> ⚠️ **免责声明**：本项目是**非官方的第三方源码公开工具**（source-available，非商业许可），与 WorkBuddy、CodeBuddy、Trae / TraeWork 及其各自权利人不存在隶属、授权或背书关系。它会读写本机第三方客户端的认证数据、按你的配置自动发起请求，并可将你的模型额度通过本地接口转发给其它工具。使用前请完整阅读[免责声明](#免责声明)，自行评估风险并确保使用方式符合相关服务条款。
...
以上项目各自遵循其自身许可；本项目为独立实现，与上述项目之间不存在隶属、合作或背书关系。相关名称与商标归各自权利人所有。
...
- 切换账号、复制会话、迁移长期记忆与连接器配置、写入 Trae 登录态等操作，会**直接修改本机第三方客户端的数据文件**（详见[数据与隐私](#数据与隐私)）。
...
- 本项目依据 [PolyForm Noncommercial License 1.0.0](./LICENSE) 许可「原样」提供，**不附带任何明示或默示的担保**，包括但不限于适销性、特定用途适用性及不侵权的默示担保。
...
本项目采用 **[PolyForm Noncommercial License 1.0.0](./LICENSE)**：**个人非商业使用是允许的，商业使用不被授权。**

允许：复制、下载、阅读与修改源码、以非商业目的分发原版或修改版，以及在个人研究、实验、学习、私人娱乐、爱好项目与业余探索中使用。慈善组织、教育机构、公共研究机构、公共安全与卫生机构、环保组织及政府机构的使用同样被允许，与经费来源无关。

**商业用途须事先取得作者的书面授权**，包括但不限于：售卖本软件或其修改版、衍生版；用于付费产品或付费服务（含订阅、按量计费）；以 SaaS / 托管服务形式对外提供；作为商业产品的捆绑组件分发；在以营利为目的的经营活动中使用（含企业内部商业项目）。

如需商业授权，请通过本仓库 Issues 联系作者。

> **版本说明**：本项目自 2026-09-22 起采用上述非商业许可。**该日期之前发布的版本仍适用其发布时附带的 MIT 许可** —— MIT 授予的权利不可撤销，本次变更不影响使用者依据旧版本已经取得的权利。
